...
REWARD_MODEL_PATH    = "./checkpoints/reward/BERT_one"
DATA_PATH            = "./data/ultrafeedback_binarized/train_gen"
OUTPUT_DIR           = "./checkpoints/RLOO_ultrafeedback_lora_BERT_one"
MAX_LENGTH           = 512
BATCH_SIZE           = 4
...
def main():
    ...
    # Ensure dataset has a "prompt" column

    # ── 6. Configure RLOOTrainer (drop unsupported generation_kwargs) ─────
    rloo_config = RLOOConfig(
        ...
        report_to="none",
        num_ppo_epochs=1,              # RLOO uses 1 PPO epoch per batch
        rloo_k=NUM_GEN_PER_PROMPT,     # K completions per prompt
        kl_coef=0.03,                  # KL penalty weight
        # generation_kwargs removed because RLOOConfig doesn't accept it
    )
